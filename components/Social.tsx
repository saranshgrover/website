...
export default function Social(): ReactElement {
	...
	return (
		<Stack direction='row' spacing={'1em'}>
			{Object.keys(socials).map(social => ({social, Icon: socialIcons[social]})).map(({social, Icon}, key) => (
				// @ts-ignore
				<Link href={socials[social]} key={key}>
					<MotionIcon whileHover={{scale: 1.2, opacity:1.2}} aria-label={social} variant="link" icon={<Icon color={color}/>} fontSize={{base:'2em', lg:'2em'}} color={color} />
				</Link>
			))}
		</Stack>
	)
